"""
BroadFSC Telegram 智能客服机器人 v4 — SOUL风格 + 销售智能 + 实时市场数据
...
11. 🆕 实时市场数据 — yfinance 接入，自动检测股票查询并注入实时行情
    • 美股/A股/港股/加密货币/外汇/大宗商品/指数
    • 2分钟缓存，中英文关键词识别
    • AI 基于 REAL DATA 回答，不再编造股价

依赖安装：
    pip install python-telegram-bot groq urllib3 numpy yfinance
...
# 🆕 实时市场数据模块
try:
    from market_data import detect_stock_query, is_stock_related, format_market_data_for_ai
            ...
            try:
                ...
                if stock_tickers:
                    ...
                    if market_context:
                        ...
                        logger.info(f"📈 Injected real-time data for: {stock_tickers}")
            except Exception as e:
                logger.warning(f"Market data injection failed (non-critical): {e}")
...
def main():
    ...
    logger.info(f"   📈 Market Data: {'yfinance ✅' if HAS_MARKET_DATA else 'Not available ❌'} | Real-time: US/A/HK/Crypto/Forex/Commodities")
    if ADMIN_CHAT_ID:
